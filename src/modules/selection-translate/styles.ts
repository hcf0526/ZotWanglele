...
export function ensureSelectionStyles(doc: Document): void {
  ...
  style.textContent = `
    ...
    .zwl-selection select, .zwl-selection button, .zwl-selection-settings select, .zwl-selection-settings input {
    ...
    .zwl-selection button { cursor: pointer; transition: background 150ms, border-color 150ms; }
    .zwl-selection button:hover { border-color: var(--zwl-red); background: #f7e7df; }
    ...
    .zwl-selection button:disabled { color: #6f6255; background: #e9dfd0; border-color: var(--zwl-rule); cursor: default; }
    ...
    @media (max-width: 760px) { .zwl-selection-settings-grid { grid-template-columns: minmax(0,1fr); } }
