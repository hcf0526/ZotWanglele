...
- 翻译菜单与仪表盘工具栏按钮已经注册。
- 任务页展示完成前的进度数值和 `100%` 状态。
- 原条目下生成 `[翻译]` 与 `[双语]` 两份附件。
- 测试结束后删除临时条目并恢复翻译设置。

`startup.test.ts` 还会检查文献综述、标题翻译菜单、“标题译文”自定义列、提示词内置名称
和右侧笔记预览相关界面挂载状态。`features.test.ts` 覆盖 Extra 字段的译文写入与更新、
AI 返回文本清理、综述来源编号、自定义提示词的新增编辑删除，以及阅读笔记标签和查找。
