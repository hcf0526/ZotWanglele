...
`selection-history.test.ts` 覆盖持久历史重读、数量上限、删除与清空、历史命中零新增请求、
按原文汇总多方案结果、词数与字数范围、9/10 词边界、规则顺序、失败方案重试、
取消全部未完成请求和存储失败提示。使用实际服务适配与模拟传输验证 Google、
Chat Completions 和 Responses 共同翻译、供应商/模型显示名称和单模型变更后的请求次数。

`selection-layout.test.ts` 检查实际 Zotero 仪表盘中的对比选项与范围控件，以及
320px、240px 工具栏中的长模型名称、历史列表和输入焦点。窗口截图保存于
`.scaffold/test/profile/selection-*-v018.png`，用于人工检查布局。测试使用模拟配置与
独立历史存储，结束后恢复原有偏好设置。
2026-09-13：0.1.8 开发版回归共 69 项通过，PDF 服务端流程按环境条件跳过。

2026-09-14：长度方案界面调整后，独立浏览器检查通过 16 项，覆盖逐项添加服务、
AI 模型选择、空方案与无效范围校验、保存重读、9/10 词边界、历史复用及未匹配时
零请求；检查了 1000px 和 520px 页面布局。构建、格式与类型检查通过。
本次保持 Zotero 关闭；上述 69 项属于调整前的原生回归，当前原生交互由用户验证。

同日追加检查通过 29 项，新增覆盖上下两组按钮、服务排序、删除模型提示、缓存设置
持久化、UTF-8 字节计算、容量清理和超大条目处理。`selection-history.test.ts` 增加
缓存容量在记录、重读和设置变更时的回归用例。本轮保持 Zotero 关闭。
